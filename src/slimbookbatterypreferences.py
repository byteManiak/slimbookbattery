...
class InfoPageGrid(BasePageGrid):
    ...
    def setup_description(self):
        ...
        msg = "<span>{}\n{}</span>".format(
            _(
                "Slimbook Battery is a battery optimization application for "
                "portable devices and can increase the battery"
            ),
            _(
                "life up to 50%. For this purpose, the third-party software is "
        ...
        thanks.set_markup(
            ...
            + (
                _(
                    "Special thanks to TLP (© 2019, linrunner), Nvidia, AMD and Intel "
                    "for offering us the necessary tools to make it possible!"
                )
            )
            + "</span>"
        )
        ...
        line.set_markup(
            ...
            + (
                _(
                    ...
                    "consider saying it in our social networks or even buy a SLIMBOOK ;)"
                )
            )
            + "</span>"
        )
    ...
    def setup_disclaimer(self):
        ...
        msg = "<span><b>{}</b>{}\n{}</span>".format(
            _("IMPORTANT NOTE:"),
            _(
                " If you have any software, widget or application that changes the CPU profile, battery"
            ),
            _(
                "optimization or similar, it may affect the operation of this application. "
    ...
    def setup_links(self):
        ...
        link = Gtk.LinkButton(
            uri="https://slimbook.es/", label=(_("Visit SLIMBOOK website"))
        )
        link.set_name("link")
        ...
        link = Gtk.LinkButton(
            uri=(_("strurlwebsite")),
            label=(_("Tutorial to learn to use Slimbook Battery")),
        )
        link.set_name("link")
        link.set_halign(Gtk.Align.CENTER)
        box.add(link)
        ...
        link = Gtk.LinkButton(
            uri="https://github.com/slimbook/slimbookbattery/tree/main/src/translations",
        ...
        msg = "<span>{}<a href='https://www.patreon.com/slimbook'>Patreon</a>{}</span>".format(
            _(
                "If you want you can support the developement of this app "
    ...
    def setup_licence(self):
        ...
        licence.set_markup(
            _("The software is provided  as is , without warranty of any kind.")
        )
    ...
    def manage_events(self, button, *args):
        ...
        if response == 1:
            path = os.path.join(
                ...
                f'report_{time.strftime("%d-%m-%y_%H:%M")}.txt',
            )
...
class GeneralGrid(BasePageGrid):
    ...
    FIELDS = [
        ...
        },
        {
            ...
            "type": "switch",
            "help": _(
                "Note: If you have the autostart activated and the icon hide, "
        ...
        },
        {
            ...
            "type": "switch",
            "help": _(
                "Note: It is very important not to use your laptop always connected to AC, "
    ...
    def setup(self):
                ...
                if link:
                    ...
                    label.set_markup(
                        "<a href='{link}'>{text}</a>".format(
                            link=link, text=_("Learn more about TDP Controller")
                        )
                    )
                    label.set_name("link")
                    label.set_halign(Gtk.Align.START)
        ...
        buttons_grid = Gtk.Grid(
            column_homogeneous=True, column_spacing=30, row_spacing=20
        )
        ...
        for column, data in enumerate(self.MODES):
            ...
            toggle = Gtk.RadioButton.new_with_label_from_widget(
                base_toggle, data.get("label")
            )
    ...
    def complete_values(self):
        ...
        self.autostart_initial = os.path.isfile(
            os.path.join(HOMEDIR, ".config/autostart/slimbookbattery-autostart.desktop")
        )
    ...
    def save_selection(self):
        ...
        if active is not None:
            ...
            if work_mode and work_mode != self.work_mode:
                ...
                code, msg = subprocess.getstatusoutput(
                    f"pkexec slimbookbattery-pkexec change_config TLP_DEFAULT_MODE {work_mode}"
                )
        ...
        if self.autostart_initial:
            destination = os.path.join(HOMEDIR, ".config/autostart")
            ...
            if not os.path.isfile(
                os.path.join(destination, "slimbookbattery-autostart.desktop")
            ):
...
class SettingsGrid(BasePageGrid):
    ...
    PERSIST_FIELDS = [
        ...
        },
        {
            ...
            "type": "switch",
            "help": _(
                "Note: Set autosuspend mode for all USB devices upon system start or a change of power source. "
        ...
        },
        {
            ...
            "type": "usb",
            "help": _(
                "Note: You need to write in the Text Box the USB IDs "
    ...
    def __init__(self, parent, custom_file, *args, **kwargs):
        ...
        self.custom_file_path = os.path.join(
            HOMEDIR, ".config/slimbookbattery/custom", custom_file
        )
    ...
    def setup_fields(self, start, fields, label_col, button_col):
        ...
        for row, data in enumerate(fields, start=start):
            ...
            if "help" in data:
                ...
                icon = Gtk.Image()
                icon.set_from_file(
                    os.path.join(IMAGES_PATH, data.get("icon", "help.png"))
                )
                icon.set_tooltip_text(data.get("help"))
            ...
            self.grid.attach(
                button, button_col - col_correction, row - row_correction, top, 1
            )
        return row
    ...
    def setup_battery_column(self):
        ...
        title.set_markup(
            "<big><b>"
            + (_("Battery mode parameters (disabled when you connect AC power):"))
            + "</b></big>"
        )
    ...
    def complete_values(self):
        ...
        value = config.getint(
            "SETTINGS", self.SECTION_MAPPING[self.custom_file]["limit_cpu"]
        )
        ...
        button.set_value(
            config.getint(
                "SETTINGS", self.SECTION_MAPPING[self.custom_file]["brightness"]
            )
        )
        for switch in ["graphic", "brightness_switch", "animations"]:
            ...
            button.set_active(
                config.getboolean(
                    "SETTINGS", self.SECTION_MAPPING[self.custom_file][switch]
                )
            )
        ...
        active_mode = None
        if governor in ["intel_pstate", "amd-pstate-epp", "amd_pstate_epp"]:
        ...
        button.set_active("wifi" in lan_disconnect and "wifi" in lan_connect)
        ...
        usb_blacklist = usb_blacklist[
            usb_blacklist.find("=") + 1 : usb_blacklist.find("\n")
        ]
    ...
    def manage_events(self, button, *args):
        ...
        if option:
            if name in ["graphic", "brightness_switch", "animations"]:
                config.set("SETTINGS", option, "1" if button.get_active() else "0")
            elif name == "brightness":
                config.set("SETTINGS", option, str(int(button.get_value())))
            elif name == "limit_cpu":
                active = button.get_active_iter()
                if active is not None:
                    model = button.get_model()
    ...
    def save_selection(self):
        ...
        if active is not None:
            ...
            if "{search}={value}".format(search=search, value=value) not in content:
                cmd = base_cmd.format(
                    search=search, value=value, file=self.custom_file_path
                )
                code, msg = subprocess.getstatusoutput(cmd)
                logger.info(
                    f'[{self.custom_file}] Setting {search} saving to "{value}" --> Exit({code}): {msg}'
                )
        ...
        for key, search_items in {
            ...
            if search_items == "WIFI_PWR_ON_BAT":
                value = (
                    "on"
                    if button.get_active() and TLP_CONF != "/etc/default/tlp"
                    else "off"
                )
            else:
        ...
        for row, data in enumerate(self.FIELDS):
            ...
            if button_type == "switch":
                ...
                button.set_adjustment(
                    Gtk.Adjustment.new(0, 0, data.get("lenght"), 5, 5, 0)
                )
                button.set_digits(0)
                button.set_hexpand(True)
...
class Preferences(Gtk.ApplicationWindow):
    ...
    def set_ui(self):
        ...
        restore_values = Gtk.Button(
            label=(_("Restore default values")),
        ...
        btn_cancel = Gtk.Button(
            label=(_("Cancel")), valign=Gtk.Align.END, halign=Gtk.Align.END
        )
        btn_cancel.set_name("cancel")
        btn_cancel.connect("clicked", self.manage_events, "x")

        btn_accept = Gtk.Button(
            label=(_("Accept")), valign=Gtk.Align.END, halign=Gtk.Align.END
        )
        ...
        if not os.path.isfile(os.path.join(CONFIG_FOLDER, "default/equilibrado")):
            ...
            shutil.copytree(
                os.path.join(base_folder, "custom"),
                os.path.join(CONFIG_FOLDER, "custom"),
            )
            shutil.copytree(
                os.path.join(base_folder, "default"),
                os.path.join(CONFIG_FOLDER, "default"),
            )
        ...
        if SESSION_TYPE and SESSION_TYPE == "x11":
            ...
            close = Gtk.Image.new_from_pixbuf(
                GdkPixbuf.Pixbuf.new_from_file_at_scale(
        ...
        check.set_tooltip_text(
            _("Style will be changed once you reopen the preferences window.")
        )
        style = (
            config.get("CONFIGURATION", "style")
        ...
        notebook.append_page(
            self.general_page_grid.get_page(),
            Gtk.Label.new(self.general_page_grid.tab_name),
        )

        for data in self.CONFIG_TABS:
            setattr(self, data.get("attr"), SettingsGrid(self, data.get("filename")))
            page_grid = getattr(self, data.get("attr"))
            notebook.append_page(page_grid.get_page(), Gtk.Label.new(data.get("title")))

        cycles_page_grid = CyclesGrid(self)
        notebook.append_page(
            cycles_page_grid.get_page(), Gtk.Label.new(cycles_page_grid.tab_name)
        )

        battery_page_grid = BatteryGrid(self)
        notebook.append_page(
            battery_page_grid.get_page(), Gtk.Label.new(battery_page_grid.tab_name)
        )

        info_page_grid = InfoPageGrid(self)
        notebook.append_page(
            info_page_grid.get_page(), Gtk.Label.new(info_page_grid.tab_name)
        )
    ...
    def check_recommendations(self):
        ...
        def check_tlp_version():
            data = {
                ...
                "info": _(
                    "We recommend you to add TLP repository, to get the last version of TLP ;)"
                ),
                "btn_label": _("Add repository"),
                "command_lbl": _(
                    "Adding linrunner/TLP oficial repository.\nMake sure you have internet connection.\n"
                ),
            ...
            if (
                not utils.get_version(TLP_VERSION) >= utils.get_version("1.5")
                and config.getboolean("CONFIGURATION", "add-tlp-repository-alert")
                and code != 0
                and VTE_VERSION[0] == 0
            ):
                show_alert(data)
    ...
    def InputToTerm(self, command):
        ...
        if VTE_VERSION[0] == 0:
            try:
                ...
                print(
                    "Failed to get current Vte version (except):",
                    VTE_VERSION[0],
                    VTE_VERSION[1],
                )
        else:
...
class PreferencesDialog(Gtk.Dialog):
    def __init__(self, parent, data):
        ...
        Gtk.Dialog.__init__(self, title="", parent=parent, flags=0)
...
def reboot_indicators(mode=None):
    ...
    exit, msg = utils.reboot_process(
        ...
        os.path.join(
            CURRENT_PATH, "slimbookbatteryindicator.py"
        ),  # Path to start indicator
    )
    if exit != 0:
        logger.error(msg)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        filename="/home/slimbook/Escritorio/logfile.log",
        filemode="w",
        format=formatter,
        level=logging.ERROR,
    )
